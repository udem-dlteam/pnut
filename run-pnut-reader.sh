...
./$TEMP_DIR/pnut.exe $PNUT_SH_OPTIONS -DDEBUG_GETCHAR "pnut.c" > "$TEMP_DIR/pnut-tokenizer.sh"

bootstrap_with_shell() {

  echo "Tokenizing with $1"

  time $1 "$TEMP_DIR/pnut-tokenizer.sh" $PNUT_SH_OPTIONS "pnut.c" > "$TEMP_DIR/pnut-read.c"

  wc pnut.c "$TEMP_DIR/pnut-tokenizer.sh" "$TEMP_DIR/pnut-read.c"
}
