...
with_size() {
  ...
  env_size=$2
  option=$3
  gcc -E -P -DARR_SIZE=$env_size $DIR/tak.c > $COMP_DIR/tak-$env_size.c

  ./benchmarks/pnut-sh.exe -D$option $COMP_DIR/tak-$env_size.c > $COMP_DIR/tak-$env_size-$option.sh

  TIME_MS=$(( `bash -c "time $shell $COMP_DIR/tak-$env_size-$option.sh" 2>&1 | fgrep real | sed -e "s/real[^0-9]*//g" -e "s/m/*60000+/g" -e "s/s//g" -e "s/\\+0\\./-1000+1/g" -e "s/\\.//g"` ))
