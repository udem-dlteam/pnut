...
# Defining enum ChildEnum
_VAL=0 _NEXT=1 _LL_SIZE=2 

# Defining enum ParentEnum
_VAL=0 _NEXT=1 _LL_SIZE=2 _VAL2=3 _NEXT2=4 _LL_SIZE2=5 

# Defining struct fields of TestStruct
_VAL=0 _NEXT=1 _LL_SIZE=2 

_shadow() { # NEXT: $2
  save_loc_var VAL NEXT
  : $(( NEXT = $2 ))
  : $(( VAL = 0 ))
  : $(( VAL = 123 ))
  : $(( NEXT = 456 ))
  defstr __str_0 "VAL: %d, NEXT: %d\n"
  _printf __ $__str_0 $VAL $NEXT
  rest_loc_var $1 NEXT VAL
}
_main() {
  defstr __str_0 "VAL: %d, NEXT: %d\n"
  _printf __ $__str_0 $_VAL $_NEXT
  _shadow __ 789
  defstr __str_0 "VAL: %d, NEXT: %d\n"
  _printf __ $__str_0 $_VAL $_NEXT
}
