...
void codegen_glo_var_decl(ast node) {
  ...
  if (get_op(type) == '(') {
    ...
    if (binding == 0) cgc_add_global_fun(name_probe, alloc_glo_label(STRING_BUF(name_probe)), promote_obj_to_global(type));
    ...
    if (binding == 0) {
      cgc_add_global(name_probe, type_width(type, true, true), promote_obj_to_global(type), false);
      binding = cgc_globals;
    }
...
void codegen_glo_fun_decl(ast node) {
  ...
  if (binding == 0) {
    cgc_add_global_fun(name_probe, alloc_glo_label(STRING_BUF(name_probe)), promote_obj_to_global(fun_type));
    binding = cgc_globals;
  }
