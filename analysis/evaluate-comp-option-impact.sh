#! /bin/sh
#
# ./analysis/evaluate-comp-option-impact.sh
# This script evaluates the impact of compilation options, testing
...
for shell in ksh dash bash yash osh zsh; do
  ...
  printf_timing \
    "pnut-sh.sh (options) pnut-sh.c with $shell" \
    "$shell $TEMP_DIR/$pnut_sh_opts $PNUT_SH_OPTIONS pnut.c > /dev/null"
