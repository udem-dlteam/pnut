...
struct rib *list_tail(struct rib *lst, num i) {
  return (i == 0) ? lst : list_tail(RIB(lst->field1), i - 1);
}

struct rib *inst_tail(struct rib *lst, num i){
  return (i == 0) ? lst : inst_tail(RIB(lst->field2), i - 1);
}

obj list_ref(struct rib *lst, num i) { return list_tail(lst, i)->field0; }

obj get_opnd(obj o) {
  return (IS_NUM(o) ? RIB(list_tail(RIB(stack), NUM(o))) : RIB(o))->field0;
}
