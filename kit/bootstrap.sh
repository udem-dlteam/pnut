...
if [ ! -e "pnut-exe" ]; then
  ...
  printf "Bootstrapping minimal pnut-exe from pnut-sh.sh\n"
  time $BOOTSTRAP_SHELL pnut-sh.sh pnut.c $PNUT_EXE_OPTIONS -DPNUT_BOOTSTRAP > pnut-exe.sh
  # 3b. Make executable version of pnut-exe. Overwrite jammed.sh to reuse its execute bit.
  printf "Making executable pnut-exe (minimal)\n"
  time $BOOTSTRAP_SHELL pnut-exe.sh pnut.c $PNUT_EXE_OPTIONS -DPNUT_BOOTSTRAP -o jammed.sh
  # Compile complete variant of pnut-exe with minimal pnut-exe (named jammed.sh)
  printf "Making executable pnut-exe (complete)\n"
  time ./jammed.sh pnut.c $PNUT_EXE_OPTIONS -DBOOTSTRAP_TCC -o pnut-exe
...
time $BOOTSTRAP_SHELL ./jammed-no-exec.sh --force-no-exec
...
# Undo the patches
./bintools simple-patch ${TCC_DIR}/tccpp.c     $PATCHES_DIR/array_sizeof.after $PATCHES_DIR/array_sizeof.before \
  || { printf "Failed to patch tccpp.c with array_sizeof\n"; exit 1; }
./bintools simple-patch ${TCC_DIR}/tccgen.c    $PATCHES_DIR/fix_stack_64_bit_operands_on_32_bit.after $PATCHES_DIR/fix_stack_64_bit_operands_on_32_bit.before \
  || { printf "Failed to patch tccgen.c with fix_stack_64_bit_operands_on_32_bit\n"; exit 1; }
./bintools simple-patch ${TCC_DIR}/tccgen.c       $PATCHES_DIR/float_negation.after $PATCHES_DIR/float_negation.before \
  || { printf "Failed to patch tccgen.c with float_negation\n"; exit 1; }
# ./bintools simple-patch ${TCC_DIR}/libtcc.c    $PATCHES_DIR/sscanf_TCC_VERSION.after $PATCHES_DIR/sscanf_TCC_VERSION.before \
#   || { printf "Failed to patch libtcc.c with sscanf_TCC_VERSION\n"; exit 1; }
./bintools simple-patch ${TCC_DIR}/tcc.h       $PATCHES_DIR/undefine_TCC_IS_NATIVE.after $PATCHES_DIR/undefine_TCC_IS_NATIVE.before \
  || { printf "Failed to patch tcc.h with undefine_TCC_IS_NATIVE\n"; exit 1; }
...
go() { # $1: name of bootstrap comp, $2: name of new compiler, $3: lib path (= $2 if empty)
  ...
  $CC -c portable_libc/src/crt1.c -o "$LIB_PATH/crt1.o" -I portable_libc/include
