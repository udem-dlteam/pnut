...
char_ptr ops_string;
ops_string = "LEA ,IMM ,JMP ,JSR ,BZ  ,BNZ ,ENT ,ADJ ,LEV ,LI  ,LC  ,SI  ,SC  ,PSH ,OR  ,XOR ,AND ,EQ  ,NE  ,LT  ,GT  ,LE  ,GE  ,SHL ,SHR ,ADD ,SUB ,MUL ,DIV ,MOD ,OPEN,READ,CLOS,PRTF,MALC,FREE,MSET,MCMP,EXIT,";
...
enum A(){ LEA ;IMM ;JMP ;JSR ;BZ  ;BNZ ;ENT ;ADJ ;LEV ;LI  ;LC  ;SI  ;SC  ;PSH ;
  ...
  while (tk >= lev) {
    ...
    if (tk == Assign) {
      ...
      *++e = (tk == Inc) ? SUB : ADD;
      ...
      if (t > PTR) { *++e = PSH; *++e = IMM; *++e = sizeof; *++e = MUL;  }
...
int main(int argc, char_ptr argv)
{
  ...
  *--sp = t;
  ...
  while (1) {
    ...
    else if (i == PRTF) { t = sp + pc[1]; a = printf(t[-1], t[-2], t[-3], t[-4], t[-5], t[-6]); }
