...
void large_int_mul(int value) {
  ...
  // Note: large_int_add may increment large_int_i but we don't want to multiply
  // the carry so it's safe. Note that we loop from the largest component first
  // to avoid multiplying the carry. This saves us from having a separate buffer
  // for the carry, but in some pathological cases, it we may end up traversing
  // the components N times, for a complexity of O(N^2)
  int i = large_int_i;
  int carry = 0;
...
int large_int_to_int32() {
  ...
  if (large_int[1] >= 256 || large_int[2] != 0 || large_int[3] != 0) {
    large_int_printf();
