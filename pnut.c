...
#ifdef EXIT_WITH_STACKTRACE
#include <execinfo.h>
// Compile with gcc -g -rdynamic to enable stack trace printing
void print_stacktrace_and_exit() {
  void *array[255];
  size_t size;
  size = backtrace(array, 255);
  fprintf(stderr, "Stack trace:\n");
  // Exclude the first entry, which is this function
  backtrace_symbols_fd(array + 1, size - 1, STDERR_FILENO);
  exit(1);
}

#define exit(code) print_stacktrace_and_exit()
...
int main(int argc, char **argv) {
  ...
  // top of the user file: it is tokenized/parsed/codegen'd first, then EOF pops
  // back to the user file. Semantically identical to a #include at the top of
  // the input, so the runtime's bindings are available when wide operations are
  // lowered to calls. arith64.c contains no 64-bit types itself, so compiling it
  // never triggers lowering (no recursion).
