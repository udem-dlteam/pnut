...
run_test() { # file_to_test: $1
  ...
  if [ ! -f "$golden_file" ]; then
    compile_test "$file" "$(test_args_comp $file)" > "$dir/$filename.$ext" 2> "$dir/$filename.pnut.err"
    gcc "$file" $(test_args_comp $file) -o "$dir/$filename-gcc.$ext" 2> "$dir/$filename.gcc.err"
    if [ $? -eq 0 ]; then
      chmod +x "$dir/$filename.$ext"
      execute_test "$dir/$filename.$ext" "$(test_timeout $file)" "$(test_args $file)" > "$dir/$filename.output"
      $dir/$filename-gcc.$ext > "$dir/$filename-gcc.output"
