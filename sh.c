...
text comp_rvalue(ast node, int context) {
  ...
  fun_call_decl_start = use_glo_decl_ix();
  ...
  if (context == RVALUE_CTX_TEST || context == RVALUE_CTX_TEST_ELSEIF) {
    ...
    result = replay_glo_decls_inline(fun_call_decl_start, use_glo_decl_ix());
...
bool comp_loop(text cond, ast body, ast loop_end_stmt, text last_line, enum STMT_CTX stmt_ctx) {
  ...
  if (loop_end_stmt) {
    loop_binding_action_start(loop_binding) = use_glo_decl_ix();
    comp_statement(loop_end_stmt, stmt_ctx);
    undo_glo_decls(loop_binding_action_start(loop_binding));
    loop_binding_action_end(loop_binding) = use_glo_decl_ix();
  }
